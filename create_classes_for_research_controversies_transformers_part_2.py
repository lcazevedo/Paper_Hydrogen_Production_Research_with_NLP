...
####################################################################################################
# PART 2 is to perform the activities that use the VLLM model to name the classes
...
# --- Temporary Files (WHERE WE WILL READ FROM) ---
...
SAMPLE_PATH = f"{config.DATA_FOLDER}/controversies_label_classified_sample500.csv"
SUMMARY_PATH = f"{config.DATA_FOLDER}/controversies_label_classification_summary.csv"
...
def make_class_name(top_terms, cluster_index):
    ...
    system_prompt = (
        ...
        "descriptive class name (3-5 words) for a cluster of **scientific controversies or debates**, "
        "based on a list of keywords. "
        "The name should clearly summarize the **central point of disagreement** "
        "(e.g., 'Role of X in Y', 'Validity of Z Model', 'Debate on Methodology')."
    )

    user_prompt = (
        f"The most common keywords in this cluster of debates are: {terms_str}. "
        f"Based on these keywords, what is the best class name describing this controversy? "
    ...
    try:
        ...
        print(f"  [!] ERROR contacting the VLLM API: {e}")
