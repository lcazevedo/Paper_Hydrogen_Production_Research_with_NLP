####################################################################################################
# This one classified better than the vectorizer
####################################################################################################
# PART 1 is to perform activities with the sentence transformer model and then free the GPU
...
# ---------------------------------------------------------
# Creates a file with label counts
# This part I had executed manually in the first test, now I added it here to wait
df = pd.read_csv(f"{config.DATA_FOLDER}/df_data_phrases.csv")

# count occurrences of the desired column
counts = df["directions_label"].value_counts().reset_index()

# rename columns to something clearer
counts.columns = ["directions_label", "count"]

# save to a new CSV
counts.to_csv(f"{config.DATA_FOLDER}/directions_label_counts.csv", index=False)

print("File 'directions_label_counts.csv' successfully generated!")



# ---------------------------------------------------------
# CONFIGURATIONS (Same as yours)
INPUT_CSV = f"{config.DATA_FOLDER}/directions_label_counts.csv"
MODEL_NAME = "sentence-transformers/all-mpnet-base-v2"
N_CLUSTERS = 12
RANDOM_SEED = 42

# --- Temporary Files ---
# File names that Stage 1 will save for Stage 2 to read
...
def main_stage1():
    ...
    labels = df["directions_label"].astype(str).tolist()
    ...
    for c in range(N_CLUSTERS):
        ...
        terms = closest["directions_label"].str.split().explode().value_counts().head(6).index.tolist()
